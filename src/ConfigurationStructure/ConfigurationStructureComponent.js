...
import {
  ...
  TYPE_INTEGER, TYPE_SECONDS, TYPE_STRING, TYPE_TIME_DELTA
...
const ConfigurationStructureComponent = ({configurationStructure}: Props) => {
  ...
  const configurationMapFunction = (directive: ConfigurationDirective, key: number) => {
    ...
    if (directive.type === TYPE_INTEGER) {
      element = <input style={inputStyles} className="pt-input" type="number"/>

    } else if (directive.type === TYPE_STRING) {
      element = <input style={inputStyles} className="pt-input" type="text"/>

    } else if (directive.type === TYPE_TIME_DELTA) {
      element = <input style={inputStyles} className="pt-input" type="number"/>
      suffix = 'minutes'

    } else if (directive.type === TYPE_SECONDS) {
      element = <input style={inputStyles} className="pt-input" type="number"/>
      suffix = 'seconds'
    }

    return <Box key={key}>
      <FormItemComponent labelSize={200} label={directive.title} element={element} suffix={suffix}/>
    </Box>
  }
